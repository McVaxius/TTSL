...
def open_path_in_shell(path: str) -> None:
    if os.name == "nt":
...
def open_screenshot_cache_folder() -> tuple[bool, str]:
    try:
        os.makedirs(SCREENSHOT_CACHE_ROOT, exist_ok=True)
        normalized = os.path.normpath(SCREENSHOT_CACHE_ROOT)
        open_path_in_shell(normalized)
        log_event(f"Opened screenshot cache folder: {normalized}")
        return True, f"Opened screenshot folder on the server host: {normalized}"
    except Exception as exc:
        log_event(f"Failed to open screenshot cache folder: {exc}")
        return False, f"Failed to open screenshot folder: {exc}"
...
CCTV_CACHE_ROOT = os.path.join(CACHE_ROOT, "cctv")
...
class LodestonePortraitCache:
    ...
    def decorate_snapshot(self, snapshot: dict) -> None:
        ...
        for party in snapshot.get("aggregateParties") or []:
            ...
            for member in party.get("members") or []:
                lookup_world = member.get("worldName")
                if member.get("isStranger") and not str(lookup_world or "").strip():
                    lookup_world = party.get("sourceWorldName")
                member["lodestone"] = self.get_visual(member.get("name"), lookup_world)

    def get_visual(self, character_name: object, world_name: object) -> dict:
        raw_name, raw_world = self._normalize_identity_lookup(character_name, world_name)
        ...
        with self._lock:
            ...
            needs_assets = status == "ready" and not self._metadata_has_assets(metadata)
    ...
    @staticmethod
    def _normalize_identity_lookup(character_name: object, world_name: object) -> tuple[str, str]:
        raw_name = " ".join(str(character_name or "").strip().split())
        raw_world = str(world_name or "").strip()
        if "@" in raw_name:
            split_name, split_world = raw_name.split("@", 1)
            raw_name = split_name.strip()
            if not raw_world:
                raw_world = split_world.strip()
...
class TTSLStateStore:
    ...
    def queue_remote_action(self, payload: dict) -> tuple[bool, str]:
        ...
        with self._lock:
            ...
            if action_type == "echocommand":
                ...
                capture_mode = str(payload.get("captureMode") or "screenshot").strip().lower()
                capture_quality = str(payload.get("captureQuality") or "medium").strip().lower()
                if capture_mode == "cctv":
                    if not bool(policy.get("allowCctvStreaming")):
                        return False, "That client does not allow web CCTV streaming."
                elif not bool(policy.get("allowScreenshotRequests")):
                ...
                queue_item = {
                    ...
                    "captureMode": "cctv" if capture_mode == "cctv" else "screenshot",
                    "captureQuality": capture_quality if capture_quality in {"low", "medium", "high"} else "medium",
                    "queuedAtUtc": utc_iso(now),
                }
                message = "Queued CCTV frame request." if queue_item["captureMode"] == "cctv" else "Queued screenshot request."
            else:
                return False, f"Unsupported action type: {action_type or 'missing'}"

            queue = self._pending_actions.setdefault(target_key, [])
            queue.append(queue_item)
            if queue_item.get("captureMode") != "cctv":
    ...
    def save_uploaded_screenshot(self, payload: dict) -> tuple[bool, str, dict | None]:
        ...
        if capture_mode == "cctv":
            ...
            file_name = f"{safe_stem}_cctv{extension}"
            file_path = os.path.join(CCTV_CACHE_ROOT, file_name)
            os.makedirs(CCTV_CACHE_ROOT, exist_ok=True)
        else:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            extension = ".jpg" if content_type == "image/jpeg" else ".png"
            file_name = f"{safe_stem}_{timestamp}{extension}"
        ...
        if capture_mode == "cctv":
            screenshot_info["quality"] = capture_quality if capture_quality in {"low", "medium", "high"} else "medium"

        with self._lock:
            client = self._clients.get(target_key)
            if client is not None:
                if capture_mode == "cctv":
                    client["lastCctvFrame"] = screenshot_info
                else:
                    client["lastScreenshot"] = screenshot_info

        if capture_mode != "cctv":
            log_event(f"Stored uploaded screenshot for {self._format_key(target_key)}: {file_name}")
        return True, "CCTV frame stored." if capture_mode == "cctv" else "Screenshot stored.", deepcopy(screenshot_info)
    ...
    def _build_aggregate_party(
        ...
        return {
            ...
            "sourceLastCctvFrame": deepcopy(source_client.get("lastCctvFrame")),
    ...
    def _build_monitored_member(
        ...
        return {
            ...
            "lastCctvFrame": deepcopy(client.get("lastCctvFrame")),
        ...
        def do_POST(self) -> None:  # noqa: N802
            try:
                ...
                if self.path == "/api/queue-action":
                    ...
                    return self._write_json({"ok": ok, "message": message, "error": None if ok else message}, HTTPStatus.OK if ok else HTTPStatus.CONFLICT)
                if self.path == "/api/open-screenshot-folder":
                    ok, message = open_screenshot_cache_folder()
                    return self._write_json({"ok": ok, "message": message, "error": None if ok else message}, HTTPStatus.OK if ok else HTTPStatus.CONFLICT)
